...
bool set_led(int led_no, bool on)
{
...
bool toggle_led(int led_no)
{
    return VALID_LED(led_no) && set_led(led_no, !led_is_on[led_no]);
}

bool blink_led(int led_no, uint32_t on_ms, uint32_t off_ms)
{
    bool ok = VALID_LED(led_no);
    if (ok) {
        if (blink_enabled && (on_ms == 0 || led_no != curr_blink_led)) {
            ...
            set_led(curr_blink_led, false);
        }
        if (on_ms > 0) {
            blink_ms[0] = on_ms;
            blink_ms[1] = off_ms;
            curr_blink_led = led_no;
            if (!blink_started) {
