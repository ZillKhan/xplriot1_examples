...
You can then select another of the examples by clicking on the "blink" text in the lower right corner of the Visual Studio Code window and then select one of the other examples in the selection list. This can also be done via *ctrl-shift-p* and then enter "C/C++: Select a configuration". Once selected chose "Build and run example" again.
...
The --example option is used to specify which example to build. If not specified the blink example will be used. See the directory structure underneath the examples subdirectory for the names available. Example:
